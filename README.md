...
- **Variabilitätsanalyse:** Im Adminbereich können gespeicherte Feedback-Fälle aus der Supabase-Tabelle `feedback_gpt` per ID geladen und mehrfach neu berechnet werden. Die Einstellungen erlauben, eine frei wählbare Anzahl von Durchläufen im ChatGPT- oder im kombinierten ChatGPT+AMBOSS-Modus zu starten. Alle benötigten Variablen werden automatisch aus dem Datensatz übernommen; fehlende Felder werden als Hinweis ausgegeben und neutral an den Prompt weitergereicht. Die Ergebnisse landen in einer eigenen Supabase-Tabelle (siehe unten) und können zusätzlich als PDF heruntergeladen werden (pro Seite genau ein Feedback, Modus ausgeblendet für verblindete Auswertungen).
    ...
    - **PDF-Stabilität:** Sehr lange URLs oder untrennbare Wörter werden vor dem Schreiben ins PDF automatisch umbrochen und zusätzlich in kleinere Token zerlegt, damit die FPDF-Bibliothek nicht mit "Not enough horizontal space to render a single character" abbricht. Sonderzeichen (z. B. Index/Tiefstellungen wie „₂“) werden über eine Unicode-Schriftart (DejaVuSans aus `/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf`) unterstützt; falls der Font im Deployment fehlt, meldet das Admin-UI dies ausdrücklich. Falls dennoch ein Platzfehler auftritt, können die Ränder in `module/admin_feedback_variation.py` testweise erhöht oder Debug-Ausgaben im PDF-Helfer aktiviert werden. Sollte ein Umbruchproblem trotz allem bestehen, kann als letzter Ausweg ein alternativer Export (z. B. HTML/Markdown) ergänzt werden, um die Feedbacks verlustfrei zu sichern. Falls `pdf.output(dest="S")` in der jeweiligen fpdf-Version keinen klassischen `bytearray`/`bytes`-Wert zurückliefert, hilft ein temporäres `st.write(type(roh_output))` im PDF-Helfer, um den beobachteten Typ zu identifizieren und die Version ggf. anzupassen.
